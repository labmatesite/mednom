...
function AddRow(userid) {
    ...
    cnt++;
    var newRow = jQuery('<tr id="addrowid' + cnt + '"></tr>'); 
    // add new tr with dynamic id and then add new row with textfield.
    jQuery('table.authors-list').append(newRow);
    
      $.ajax({
            type: "GET",
            url: "addnewrow.php",
            data: "cnt=" + cnt+ "&username=" + userid,
            cache: false,
...
function RemoveRow(rowno) {
    ...
    $("#product_name" + rowno).each(function () {
...
//alert(val2);
        ...
        $.ajax({
            ...
            success: function (html) {
                ...
                $("#product_desc" + cnt).html(html);
                $("#prod_sku" + cnt).val(val2);
...
function product_Price(val1, cnt,username,price) {
...
//alert("jugal");
    ...
    $.ajax({
        type: "GET",
        url: "viewproductPrice.php",
        data: "val1=" + val2 + "&cnt1=" + cnt + "&price=" + price+ "&username=" + username,
        cache: false,
        success: function (html) {
            $("#selling_price" + cnt).val(html);
...
function sord(sp, cnt) // Selling Price field
{
    ...
    var tot = qty * Number(sp.replace(/[^0-9\.]+/g, ""));
    ...
    while (i <= numItems)
    {
        ...
        {
            var bank_charges = 30.00;
            var pre_tax = +x + +bank_charges+ +Number(sc.replace(/[^0-9\.]+/g, ""));
        ...
        else if (discount_final == "r_perprice")
        {
            ...
            var f_x = x - x2;
            var bank_charges = 30.00;
            var pre_tax = +f_x + +bank_charges+ +Number(sc.replace(/[^0-9\.]+/g, ""));
...
function DiscFinal(frm) { // Discount Module Calculation
    ...
    if (frm == "zero")
    {
        ...
        var pre_tax = +tot + +bank_charges+ +Number(sc.replace(/[^0-9\.]+/g, ""));
        ...
        else
        {
            ...
            var pre_tax = +f_tot + +bank_charges+ +Number(sc.replace(/[^0-9\.]+/g, ""));
...
function tax(txid, txvalue, txstatus, txname) // taxes 
{
    ...
    if (txstatus == "active")
    {
        ...
        document.getElementById('st' + txid).onclick = function () {
            tax(txid, txvalue, "deactive", txname);
        };
    }
    else if (txstatus == "deactive")
    {
        taxes2 = +taxes1 + +txvalue;
        document.getElementById('st' + txid).onclick = function () {
            tax(txid, txvalue, "active", txname);
        };
    }
